   ...
   //Check if any credentials are given or show the basic config.
    if ((!empty($CONFIG["acumulus_code"])) && (!empty($CONFIG["acumulus_username"])) && (!empty($CONFIG["acumulus_password"]))){
        ...
        if ((string)$xml->general->about){
            ...
            array_push($configarray["fields"], array("FriendlyName" => "Credential check", "Description" => '<font color="red"><b>'.'The credentials are not correct.'.'</b></font>'));
            return $configarray;
        }
   }else{
        // one or more required credentials are not given.
        $configarray = acumulus_connect_construct_basic_configarray();
        array_push($configarray["fields"], array("FriendlyName" => "Credential check", "Description" => '<font color="blue"><b>'.'Please enter your credentials and click "Save Changes", to continue configurating.'.'</b></font>'));
        return $configarray;
   }
}
...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------
   Helper functions
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------*/
function acumulus_connect_construct_basic_configarray(){                // Helper function to create the basic configarray  (used to enter credentials)
    $configarray = array(
        ...
        "language" => "english",
        "fields" => array(
            "acumulus_code" => array ("FriendlyName" => "Contract code", "Type" => "text", "Size" => "25", "Description" => "Enter the Acumulus&reg; contract code here.", "Default" => "", ),
            "acumulus_username" => array ("FriendlyName" => "Username", "Type" => "text", "Size" => "25", "Description" => "Enter the Acumulus&reg; username here.", "Default" => "", ),
            "acumulus_password" => array ("FriendlyName" => "Password", "Type" => "password", "Size" => "25", "Description" => "Enter the Acumulus&reg; password here.", ),
            )
        );
   return $configarray;
}
function acumulus_connect_construct_full_configarray(){                 // Helper function to create the full configarray (used after credentials are correct)
    $CONFIG = acumulus_connect_getConfig();
    $configarray = acumulus_connect_construct_basic_configarray();
    ...
    // * Features                                                              *
    // *************************************************************************
    $configarray["fields"]["acumulus_features_hook_invoice_message1"]   =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("Features"),
                                                                                 );
    $configarray["fields"]["acumulus_emailaspdf"]                       =   array ("FriendlyName" => "Let Acumulus&reg send invoice",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    $configarray["fields"]["acumulus_customer_import_enabled"]          =   array ("FriendlyName" => "Import customer details",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    $configarray["fields"]["acumulus_hook_invoice_create_enabled"]  =   array ("FriendlyName" => "Enable create hook",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
                                                                                ...
                                                                                "Default" => "on");                                                    
    $configarray["fields"]["acumulus_hook_invoice_paid_enabled"]    =   array ("FriendlyName" => "Enable paid hook",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
                                                                                "Description" => "Update or Create Invoice in Acumulus&reg; directly when paid for.",
                                                                                "Default" => "on");
    $configarray["fields"]["acumulus_hook_invoice_canceled_enabled"]    =   array ("FriendlyName" => "Enable Canceled invoice hook",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    if ($CONFIG['acumulus_customer_import_enabled'] === "on"){
        $configarray["fields"]["acumulus_customer_message1"]            =   array ("FriendlyName" => "",
                                                                                    "Description" =>  acumulus_connect_newConfigSection("Customer Settings"),
                                                                                     );


        $configarray["fields"]["acumulus_customer_type"]                =   array ("FriendlyName" => "Customer Type",
                                                                                    "Type" => "dropdown",
        ...
        $configarray["fields"]["acumulus_customer_countryautoname"]     =   array ("FriendlyName" => "Customer country",
                                                                                    "Type" => "dropdown",
        ...
        $configarray["fields"]["acumulus_customer_overwriteifexists"]   =   array ("FriendlyName" => "Overwrite customer details",
                                                                                    "Type" => "yesno",
                                                                                    "Size" => "25",
                                                                                    "Description" => "Overwrite customer contact details in Acumulus&reg.",
                                                                                    "Default" => "on");
        $configarray["fields"]["acumulus_customer_disableduplicates"]   =   array ("FriendlyName" => "Disable customer duplicates",
                                                                                    "Type" => "yesno",
                                                                                    "Size" => "25",
                                                                                    "Description" => "Disable older instances of a contact in Acumulus&reg when multiple contacts match the customer email.");
        $configarray["fields"]["acumulus_whmcs_vatfield"]               =   array ("FriendlyName" => "TAX or VAT field",
                                                                                    "Type" => "dropdown",
                                                                                    ...
                                                                                    "Default" => "[VAT Number]");
        $configarray["fields"]["acumulus_whmcs_ibanfield"]               =   array ("FriendlyName" => "IBAN field",
                                                                                    "Type" => "dropdown",
                                                                                    ...
                                                                                    "Default" => "");
        $configarray["fields"]["acumulus_cusromer_mark"]               =   array ("FriendlyName" => "Client Mark",
                                                                                    "Type" => "text",
                                                                                    "Size" => "40",
    ...
    // *************************************************************************
    $configarray["fields"]["acumulus_invoice_message1"]                 =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("Standard Invoice Settings"),
                                                                                 );

    $configarray["fields"]["acumulus_invoice_default_costcenter"]       =   array ("FriendlyName" => "Default Costcenter",
                                                                                "Type" => "dropdown",
                                                                                "Options" =>  $stringCostcenters,
                                                                                "Description" => "The default costcenter the new invoices will be booked to.",
                                                                                "Default" => "");

    $configarray["fields"]["acumulus_invoice_default_nature"]           =   array ("FriendlyName" => "Default nature",
                                                                                "Type" => "dropdown",
                                                                                "Options" =>  'Product,Service',
                                                                                "Description" => "The default Nature on wich an invoice line is booked.",
                                                                                "Default" => "Service");

    $configarray["fields"]["acumulus_use_acumulus_invoice_numbering"]   =   array ("FriendlyName" => "Use Acumulus&reg invoice numbering",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
                                                                                "Description" => "When enabled, WHMCS ignors the invoice number and uses Acumulus&reg sequential invoice numbering.<br>",
                                                                                "Default" => "on");

    $configarray["fields"]["acumulus_invoice_description"]              =   array ("FriendlyName" => "Invoice title",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
    ...
    $configarray["fields"]["acumulus_creditinvoice_description"]        =   array ("FriendlyName" => "Credit Invoice title",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
    ...
    $configarray["fields"]["acumulus_invoice_descriptiontext"]          =   array ("FriendlyName" => "Invoice extended description",
                                                                                "Type" => "textarea",
                                                                                "Rows" => "4",
                                                                                "Cols" => "47",
    ...
    $configarray["fields"]["acumulus_invoice_invoicenotes"]             =   array ("FriendlyName" => "Invoice additional remarks",
                                                                                "Type" => "textarea",
                                                                                "Rows" => "4",
                                                                                "Cols" => "47",
    ...
    $configarray["fields"]["acumulus_invoice_template"]                 =   array ("FriendlyName" => "Invoice Template",
                                                                                "Type" => "dropdown",
    ...
    // *************************************************************************
    $configarray["fields"]["acumulus_invoice_message2"]                 =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("Additional Invoice Settings"),
                                                                                 );

    $configarray["fields"]["acumulus_summarize_invoice"]                =   array ("FriendlyName" => "Summarize invoice lines",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    $configarray["fields"]["acumulus_invoice_correction"]               =   array ("FriendlyName" => "Invoice Correction",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    if ($CONFIG['acumulus_invoice_correction'] == 'on'){
        $configarray["fields"]["acumulus_invoice_correction_text"]     =   array ("FriendlyName" => "Invoice correction line description<br>",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
    ...
    if ($CONFIG['acumulus_summarize_invoice'] == 'on'){
        $configarray["fields"]["acumulus_summarization_text_taxed"]     =   array ("FriendlyName" => "Invoice line Sumarization description<br>including TAX",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
        ...
        $configarray["fields"]["acumulus_summarization_text_untaxed"]   =   array ("FriendlyName" => "Invoice line Sumarization description<br>excluding TAX",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
    ...
    $configarray["fields"]["acumulus_invoice_use_last_paymentmethod"]  =   array ("FriendlyName" => "Use last paymentmethod",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    // *************************************************************************
    $configarray["fields"]["acumulus_accountnumber_message1"]           =   array ("FriendlyName" => "",
    ...
    foreach (acumulus_connect_getWHMCSAccountNumbers($CONFIG['acumulus_whmcs_admin']) as $accountNumber){
       // echo $accountNumber['module'];
        $configarray["fields"]["acumulus_AccountNumber_". $accountNumber['module']] = array ("FriendlyName" =>  "WHMCS Payment Gateway: <b>" . $accountNumber['displayname'] ."</b>",
                                                                                "Type" => "dropdown",
    ...
    if ($CONFIG['acumulus_emailaspdf'] == 'on'){
       ...
       $configarray["fields"]["acumulus_emailaspdf_message1"]           =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("Acumulus E-Mail Settings"),
                                                                                 );
        $configarray["fields"]["acumulus_emailaspdf_message2"]          =   array (  "FriendlyName" => "E-mail To",
                                                                                "Description" => "The invoice will be send to the primary customer email adres. WHMCS Aditional Contacts will be ignored.",
                                                                                );

        $configarray["fields"]["acumulus_emailaspdf_emailbcc"]          =   array ("FriendlyName" => "E-mail BCC",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
                                                                                ...
                                                                                "Default" => "", );
        $configarray["fields"]["acumulus_emailaspdf_emailfrom"]         =   array ("FriendlyName" => "Email From",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
                                                                                ...
                                                                                "Default" => "", );
        $configarray["fields"]["acumulus_emailaspdf_subject"]           =   array ("FriendlyName" => "E-mail Subject",
                                                                                "Type" => "text",
                                                                                "Size" => "40",
                                                                                ...
                                                                                "Default" => "", );
        $configarray["fields"]["acumulus_emailaspdf_message"]           =   array ("FriendlyName" => "E-mail Message",
                                                                                "Type" => "textarea",
                                                                                "Rows" => "4",
                                                                                "Cols" => "47",
                                                                                ...
                                                                                "Default" => "", );
         $configarray["fields"]["acumulus_emailaspdf_confirmreading"]   =   array ("FriendlyName" => "E-mail Confirm Reading",
                                                                                "Type" => "yesno",
                                                                                "Size" => "25",
    ...
    // *************************************************************************
    $configarray["fields"]["acumulus_warning_message2"]                 =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("Warnings & Errors"),
                                                                                );

    $configarray["fields"]["acumulus_warning_email_address"]            =   array ("FriendlyName" => "Email address for warnings",
                                                                                "Type" => "text",
                                                                                "Size" => "25",
                                                                                "Description" => "Enter a email address where api warning messages should be send to.<br> When ommitted no warnings will be send.",
                                                                                "Default" => "");

    $configarray["fields"]["acumulus_error_email_address"]              =   array ( "FriendlyName" => "Email address for errors",
                                                                                "Type" => "text",
                                                                                "Size" => "25",
    ...
    // * API Variables                                                         *
    // *************************************************************************
    $configarray["fields"]["acumulus_api_message1"]                     =   array ("FriendlyName" => "",
                                                                                "Description" =>  acumulus_connect_newConfigSection("WHMCS API Settings"),
                                                                                 );
    $configarray["fields"]["acumulus_whmcs_admin"]                      =   array ("FriendlyName" => "Hook admin",
                                                                                "Type" => "dropdown",
...
function acumulus_connect_newConfigSection($section){
     $linebreak = '';
    for ($i = 1; $i <= 100; $i++) { $linebreak .= "&diams;"; };
     $linebreak .= "<br>&diams;&nbsp;&nbsp;" . $section . "<br>";
    for ($i = 1; $i <= 100; $i++) { $linebreak .= "&diams;"; };
    return $linebreak;
}

function acumulus_connect_show_module_form($vars){                      // Helper function to show the Module form.
