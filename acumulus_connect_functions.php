...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------
   Helper functions
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------*/
function acumulus_connect_getConfig(){                                              // Helper function to load the configuration data stored in the Database.
    $result = true;
    try {
    ...
    try {
        $CONFIG['acumulus_hook_invoice_create_enabled'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_hook_invoice_create_enabled')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
        $CONFIG['acumulus_hook_invoice_paid_enabled'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_hook_invoice_paid_enabled')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
        $CONFIG['acumulus_hook_invoice_canceled_enabled'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_hook_invoice_canceled_enabled')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
    ...
    try {
        $CONFIG['acumulus_customer_overwriteifexists'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_customer_overwriteifexists')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
        $CONFIG['acumulus_customer_disableduplicates'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_customer_disableduplicates')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
    ...
    try {
        $CONFIG['acumulus_invoice_default_costcenterid'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_invoice_default_costcenter')->first()->value, 2)[0];
    } catch (\Exception $e) {
        $result = false;
    };
    try {
        $CONFIG['acumulus_invoice_default_costcentername'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_invoice_default_costcenter')->first()->value, 2)[1];
    } catch (\Exception $e) {
        $result = false;
    };

    try {
        $CONFIG['acumulus_invoice_templateid'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_invoice_template')->first()->value, 2)[0];
    } catch (\Exception $e) {
        $result = false;
    };
    try {
        $CONFIG['acumulus_invoice_templatename'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_invoice_template')->first()->value, 2)[1];
    } catch (\Exception $e) {
        $result = false;
    };

    try {
        $CONFIG['acumulus_use_acumulus_invoice_numbering'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_use_acumulus_invoice_numbering')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    ...
    try {
        $CONFIG['acumulus_summarization_text_untaxed'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_summarization_text_untaxed')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
    ...
    try {
        $CONFIG['acumulus_creditinvoice_description'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_creditinvoice_description')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    try {
    ...
    try {
        $CONFIG['acumulus_invoice_use_last_paymentmethod'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_invoice_use_last_paymentmethod')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    ...
    try {
        $CONFIG['acumulus_emailaspdf_confirmreading'] = Capsule::table('tbladdonmodules')->where('setting', 'acumulus_emailaspdf_confirmreading')->first()->value;
    } catch (\Exception $e) {
        $result = false;
    };
    ...
    foreach (acumulus_connect_getWHMCSAccountNumbers($CONFIG['acumulus_whmcs_admin']) as $accountNumber) {
        try {
            $CONFIG['account_numbers'][$accountNumber['module']]['id'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_AccountNumber_'.$accountNumber['module'])->first()->value, 2)[0];
            $CONFIG['account_numbers'][$accountNumber['module']]['name'] = explode(" ", Capsule::table('tbladdonmodules')->where('setting', 'acumulus_AccountNumber_'.$accountNumber['module'])->first()->value, 2)[1];
        } catch (\Exception $e) {
            $result = false;
        };
    };
...
function acumulus_connect_get_admins(){                                             // WHMCS API Call to retrieve administrators.
    $admins = array();
...
function acumulus_connect_getClientCustomfields(){                                  // WHMCS API Call to retrieve customfields.
...
function acumulus_connect_getclient($clientid, $adminuser){                         // WHMCS API Call to retreive client details.
...
function acumulus_connect_getinvoice($invoiceid, $adminuser, $expand = true){       // WHMCS API Call to retrieve invoice details.
    $command = 'getinvoice';
...
function acumulus_connect_getWHMCSAccountNumbers($adminuser){                       // WHMCS API Call to retrieve the accountnumbers from WHMCS.
...
function acumulus_connect_getPaymentGatewayUsed($CONFIG, $invoiceid){               // WHMCS API Call to retrieve the payment gateway used in the first transaction.
...
function acumulus_connect_getWHMCSVersion(){                                        // helper function to retrieve the current WHMCS version.
    $version = Capsule::table('tblconfiguration')->where('setting', 'Version')->first();
    return $version->value;
}
function acumulus_connect_expandInvoiceWithCustoms($invoice, $client){              // helper function to Extend the invoice with custom values for tax etc.
    ...
    if ($CONFIG['TaxType'] == 'Exclusive') {
        ...
        foreach ($invoice['items']['item'] as $item) {
            if ($item['taxed'] == 1) {
                $invoice['items']['item'][$counter]['custom_tax_unrounded']      = round((floatval($item['amount'])/100) * floatval($invoice["taxrate"]), 4);  // (amount / 100) * Tax Rate
               $invoice['items']['item'][$counter]['custom_tax_rounded']        = round((floatval($item['amount'])/100) * floatval($invoice["taxrate"]), 2);  // (amount / 100) * Tax Rate
               $invoice['items']['item'][$counter]['custom_price_incl_tax_unrounded'] = round(floatval($item['amount'] +      ((floatval($item['amount'])/100) * floatval($invoice["taxrate"]))), 4);   // amount + ((amount / 100) * Tax Rate)
               $invoice['items']['item'][$counter]['custom_price_incl_tax_rounded']   = round(floatval($item['amount'] + round((floatval($item['amount'])/100) * floatval($invoice["taxrate"]), 2)), 2); // amount + ((amount / 100) * Tax Rate)
               $invoice['items']['item'][$counter]['custom_price_excl_tax_unrounded'] = round(floatval($item['amount']), 4);
                ...
                $invoice['custom']['subtotal_taxedItems_inclTax'] += round(floatval($item['amount'] + ((floatval($item['amount'])/100) * floatval($invoice["taxrate"]))), 4);   // amount + ((amount / 100) * Tax Rate)
               $invoice['custom']['total_tax_roundedPerItem']    += round((floatval($item['amount'])/100) * floatval($invoice["taxrate"]), 2);   // amount + ((amount / 100) * Tax Rate)
               $invoice['custom']['total_tax']                   += round((floatval($item['amount'])/100) * floatval($invoice["taxrate"]), 4);   // amount + ((amount / 100) * Tax Rate)
            } else {
        ...
        foreach ($invoice['items']['item'] as $item) {
            if ($item['taxed'] == 1) {
                $invoice['items']['item'][$counter]['custom_tax_unrounded']            = round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * floatval($invoice["taxrate"]), 4);    // amount / (100 + Tax Rate)
               $invoice['items']['item'][$counter]['custom_tax_rounded']              = round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * floatval($invoice["taxrate"]), 2);   // amount / (100 + Tax Rate)
               $invoice['items']['item'][$counter]['custom_price_incl_tax_unrounded'] = round(floatval($item['amount']), 4);
                ...
                $invoice['items']['item'][$counter]['custom_price_excl_tax_unrounded'] = round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * 100, 4);  // (amount / (100 + Tax Rate)) * 100
               $invoice['items']['item'][$counter]['custom_price_excl_tax_rounded']   = round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * 100, 2);  // (amount / (100 + Tax Rate)) * 100
               $invoice['custom']['subtotal_taxedItems_exclTax'] += round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * 100, 4);  // (amount / (100 + Tax Rate)) * 100 ;
               $invoice['custom']['subtotal_taxedItems_inclTax'] += round(floatval($item['amount']), 4);
                $invoice['custom']['total_tax_roundedPerItem']    += round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * floatval($invoice["taxrate"]), 2);   // amount / (100 + Tax Rate);
                $invoice['custom']['total_tax']                   += round((floatval($item['amount'])/ (100 +floatval($invoice["taxrate"]))) * floatval($invoice["taxrate"]), 4);
            } else {
                $invoice['items']['item'][$counter]['custom_tax_unrounded']      = (float)0;
                $invoice['items']['item'][$counter]['custom_tax_rounded']        = (float)0;
                $invoice['items']['item'][$counter]['custom_price_incl_tax_unrounded'] = round(floatval($item['amount']), 4);
...
function acumulus_connect_replaceVarsInText($text, $CONFIG, $invoice, $client) {     // helper function to replace text with dynamic values
...
function acumulus_connect_getCostcenters(){                                         // Helper function to retrieve the costcenters from Acumulus.
...
function acumulus_connect_getAccounts(){                                            // Helper function to retrieve the accountnumbers from Acumulus.
    $xml = acumulus_connect_basicXML(false); //construct the basic xml without email on errors or warnings.
    $xml->addChild('format', 'xml');
    $xml_string = urlencode($xml->asXML());
...
function acumulus_connect_getTemplates(){                                           // Helper function to retrieve the invoice templates from Acumulus.
    $xml = acumulus_connect_basicXML(false); //construct the basic xml without email on errors or warnings.
    $xml->addChild('format', 'xml');
    $xml_string = urlencode($xml->asXML());
...
function acumulus_connect_isCountryinEU($countrycode, $date){                       // Helper function to check if country is a EU member.
    // $date is for future use like the brexit

    $eu_countries = array(
        'BE', 'BG', 'CY', 'DK', 'DE', 'EE', 'FI', 'FR', 'EL', 'HU', 'IE', 'IT', 'HR', 'LV', 'LT',
        'LU', 'MT', 'NL', 'AT', 'PL', 'PT', 'RO', 'SI', 'SK', 'ES', 'CZ', 'SE', 'GB', 'UK'
    );
...
function acumulus_connect_invoiceDetails_getVatType($CONFIG, $invoice, $client){       // Helper function to calculate the vattype and taxrate by country, nature MOSS etc.
    /* Vattypes:
...
function acumulus_connect_sendInvoicetoAccumulus($CONFIG, $invoice, $client, $xml){    // Helper function to send the constructed XML to Acumulus with curl.
    ...
    logModuleCall("acumulus_connect", "Send Invoice to Acumulus", $xml->asXML(), $rawdata, $result, array($CONFIG['acumulus_code'],$CONFIG['acumulus_username'],$CONFIG['acumulus_password']));
...
function acumulus_connect_setinvoicetoken($invoice, $invoicetoken, $entryid, $config){ // Helper function to update the token table for unpaid invoices.
    ...
    if ( !Capsule::table('mod_acumulus_connect')->where('id', $invoiceid)->exists() ) {
        ...
        try {
            if (Capsule::table('mod_acumulus_connect')->insert(
                ...
                logModuleCall("acumulus_connect", "Add invoicetoken to database", '', 'Added successfull: '.$invoice["invoiceid"].' - '. $invoicetoken . ' - '. $entryid, '', '');
            }
        } catch (\Exception $e) {
        ...
        try {
            ...
            if ($updatedUserCount > 0) {
                logModuleCall("acumulus_connect", "Update invoicetoken in database", '', 'Changed successfull: '.$invoice["invoiceid"].' - '. $invoicetoken, '', '');
            }
        } catch (\Exception $e) {
...
function acumulus_connect_estimateTotals($CONFIG, $invoice, $client){                  // Helper function to estimate the totals like Acumulus would calculate.
...
function acumulus_connect_getPaymentStatus($CONFIG, $invoice, $client, $token){        // Helper function to get the current payment status from Acumulus.
       $xml = acumulus_connect_basicXML();
        ...
        $rawdata = curl_exec($ch);
        $result = json_decode(json_encode((array) simplexml_load_string($rawdata)), 1);
        logModuleCall("acumulus_connect", "invoice_paymentstatus_get()", $xml->asXML(), $rawdata, $result, array($CONFIG['acumulus_code'],$CONFIG['acumulus_username'],$CONFIG['acumulus_password']));
        curl_close($ch);
        return($result['invoice']);    //   [token] => fe283169378b0f829d3a465824d8bff6 ,  [paymentstatus] => 1 ,  [paymentdate] => 2018-01-20
}
function acumulus_connect_inverseInvoiceAmounts($invoice){                             // Helper function to inverse the amounts for teh credit invoice.
    $negativeItems = array();
...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------*/
function acumulus_connect_basicXML($includeWarnings = true){                                            // Helper function to construct the basix XML.
    $CONFIG = acumulus_connect_getConfig();
    //Create The XML FILE
...
function acumulus_connect_XMLPrepairCustomerDetails($CONFIG, $invoice, $client){                        // helper function to prepair the customer data for the XML.
    ...
    $customerDetails['postalcode']           =  (isset($client['postcode'])) ? preg_replace('/\s+/', '', $client['postcode']) : '';   //Remove any whitespaces from teh postcode
...
function acumulus_connect_XMLPrepairInvoiceDetails($CONFIG, $invoice, $client, $isbulkimport = false, $iscredit = false){  // helper function to prepair the invoice data for the XML.
    ...
    $invoiceDetails['paymentdate']           =   (explode(' ', $invoice['datepaid'])[0] == '0000-00-00' ? null : explode(' ', $invoice['datepaid'])[0]);                           // Format: yyyy-mm-dd.
    ...
    $invoiceDetails['description']           =   acumulus_connect_replaceVarsInText($CONFIG['acumulus_invoice_description'], $CONFIG, $invoice, $client);      // Overall description of the invoice, invoice title.
    $invoiceDetails['descriptiontext']       =   str_replace("\n", "\\n", acumulus_connect_replaceVarsInText($CONFIG['acumulus_invoice_descriptiontext'], $CONFIG, $invoice, $client));  // Multiline field for extended description of the invoice. Content will appear on invoice and associated emails. Use \n for newlines. Tabs are not supported.
    $invoiceDetails['invoicenotes']          =   str_replace("{TAB}", "\\t", str_replace("\n", "\\n", acumulus_connect_replaceVarsInText($CONFIG['acumulus_invoice_invoicenotes'], $CONFIG, $invoice, $client)));     // Multiline field for additional remarks. Use \n for newlines and \t for tabs. Contents is placed in notes/comments section of the invoice. Content will not appear on the actual invoice or associated emails.
    ...
    // *************************************************************************
    if ($iscredit){
       $invoiceDetails['description']           =   acumulus_connect_replaceVarsInText($CONFIG['acumulus_creditinvoice_description'], $CONFIG, $invoice, $client);      // Overall description of the invoice, invoice title.
    ...
    if ($CONFIG["acumulus_summarize_invoice"] == 'on') {
        ...
        if (!empty($invoice['custom']['subtotal_taxedItems_exclTax'])) {
            array_push($invoiceDetails['invoicelines'], array(
                                                    'itemnumber' => null,        // non mandatory, If set, this number will precede the product description (product).
                                                    'product'    => acumulus_connect_replaceVarsInText($CONFIG['acumulus_summarization_text_taxed'], $CONFIG, $invoice, $client),        // non mandatory, Product or service description.
                                                    'nature'     => $CONFIG['acumulus_invoice_default_nature'] ,        // non mandatory,  'Product'( default ), 'Service'
                                                    'unitprice'  => $invoice['custom']['subtotal_taxedItems_exclTax'],        // non mandatory,  Unit price without VAT. Decimal separator is a point. No thousand separators. 4 decimals precision. E.g. 12.95 or 1200.50 or 12.6495. Will be rounded if provided with more than 4 decimals.
                                                    'vatrate'    => $invoice['custom']['taxrate'],        // mandatory,  Applicable vatrate for the product. Defaults to "21".
                                                    'quantity'   => "1",        // non mandatory, Number of products/services. Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 1 or 1.5 or 12.64. Default is 1.
                                                    'costprice'  => null,        // non mandatory, Use in case of margin vat (marge-regeling). Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 12.95 or 1200.50
                                                    ));
        }
        ...
        if (!empty($invoice['custom']['subtotal_untaxedItems'])) {
            array_push($invoiceDetails['invoicelines'], array(
                                                    'itemnumber' => null,        // non mandatory, If set, this number will precede the product description (product).
                                                    'product'    => acumulus_connect_replaceVarsInText($CONFIG['acumulus_summarization_text_untaxed'], $CONFIG, $invoice, $client),        // non mandatory, Product or service description.
                                                    'nature'     => $CONFIG['acumulus_invoice_default_nature'] ,        // non mandatory,  'Product'( default ), 'Service'
                                                    'unitprice'  => $invoice['custom']['subtotal_untaxedItems'],        // non mandatory,  Unit price without VAT. Decimal separator is a point. No thousand separators. 4 decimals precision. E.g. 12.95 or 1200.50 or 12.6495. Will be rounded if provided with more than 4 decimals.
                                                    'vatrate'    => "-1",        // mandatory,  Applicable vatrate for the product. Defaults to "21".
                                                    'quantity'   => "1",        // non mandatory, Number of products/services. Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 1 or 1.5 or 12.64. Default is 1.
                                                    'costprice'  => null,        // non mandatory, Use in case of margin vat (marge-regeling). Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 12.95 or 1200.50
                                                    ));
        }
        ...
        foreach ($invoice['items']['item'] as $item) {
            array_push($invoiceDetails['invoicelines'], array(
                                                'itemnumber' => null,        // non mandatory, If set, this number will precede the product description (product).
                                                // #2
                                                'product'    => preg_replace("/\r\n|\r|\n/", ' ', $item['description']),        // non mandatory, Product or service description.
                                                'nature'     => $CONFIG['acumulus_invoice_default_nature'] ,        // non mandatory,  'Product'( default ), 'Service'
                                                'unitprice'  => $item['custom_price_excl_tax_unrounded'],        // non mandatory,  Unit price without VAT. Decimal separator is a point. No thousand separators. 4 decimals precision. E.g. 12.95 or 1200.50 or 12.6495. Will be rounded if provided with more than 4 decimals.
                                                'vatrate'    => ($item['taxed'] == 1) ? $invoice['custom']['taxrate'] : '-1',        // mandatory,  Applicable vatrate for the product. Defaults to "21".
                                                'quantity'   => "1",        // non mandatory, Number of products/services. Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 1 or 1.5 or 12.64. Default is 1.
                                                'costprice'  => null,        // non mandatory, Use in case of margin vat (marge-regeling). Decimal separator is a point. No thousand separators. 2 decimals precision. E.g. 12.95 or 1200.50
                                                ));
        } //end foreach
...
function acumulus_connect_generatexml($CONFIG, $invoice, $client, $isbulkimport, $iscredit = false){                       // Helper function to construct the XML that will be send to Acumulus
...
function acumulus_connect_updateInvoice($CONFIG, $invoiceid, $usedate = null) {
    ...
    if (!empty($token)) {
        ...
        $rawdata = curl_exec($ch);
        $result = json_decode(json_encode((array) simplexml_load_string($rawdata)), 1);
        logModuleCall("acumulus_connect", "Update Invoice paid status in Acumulus", $xml->asXML(), $rawdata, $result, array($CONFIG['acumulus_code'],$CONFIG['acumulus_username'],$CONFIG['acumulus_password']));
        ...
        logModuleCall("acumulus_connect", "Hook[InvoicePaid][acumulus_connect_updateInvoice]", '', 'Token for id : "'.$invoice["invoiceid"].'" not found so sending entire invoice.', '', '');
...
function acumulus_connect_updateInvoicePaymentMethode($CONFIG, $invoiceid, $paymentmethod){
    $entryid = Capsule::table('mod_acumulus_connect')->where('id', $invoiceid)->value('entryid');
      ...
      if (!empty($entryid)){
        ...
        $rawdata = curl_exec($ch);
        $result = json_decode(json_encode((array) simplexml_load_string($rawdata)), 1);
        logModuleCall("acumulus_connect", "Update accountnumber in invoice", $xml->asXML(), $rawdata, $result, array($CONFIG['acumulus_code'],$CONFIG['acumulus_username'],$CONFIG['acumulus_password']));
